...
class RNN2captcha(object):
    ...
    def __init__(self):
        ...
        self.diminput = self.image_width
        self.dimhidden = 128
        self.dimoutput = self.max_captcha * self.char_set_len
        self.nsteps = self.image_height

        self.batch_size = 10
        self.display_step = 100
        self.total_train_times = 20000

    # 定义网络
    def RNN(self, X, W, b, nsteps, diminput, dimhidden):
        ...
        # 搭建LSTM层（看成隐层）
        # 有lstm_size个单元
        lstm = tf.contrib.rnn.BasicLSTMCell(self.dimhidden)
        # dropout
        drop = tf.contrib.rnn.DropoutWrapper(lstm, output_keep_prob=keep_prob)

        cell = tf.contrib.rnn.MultiRNNCell([tf.contrib.rnn.BasicLSTMCell(self.dimhidden) for _ in range(1)])
        ...
        outputs = tf.reshape(outputs, [-1, self.dimhidden])
        # 得到输出, logits大小为[batch_size*binary_dim, 1]
        logits = tf.sigmoid(tf.matmul(outputs, W["h2"]))
        # [batch_size*binary_dim, 1] ==> [batch_size, binary_dim]
        predictions = tf.reshape(logits, [-1, self.image_height])
    ...
    def start_train(self):
        ...
        W = {
            "h1": tf.Variable(tf.random_normal([self.diminput, self.dimhidden])),  # [28, 128]
            "h2": tf.Variable(tf.random_normal([self.dimhidden, self.dimoutput]))  # [128, 10]
        }

        b = {
            "b1": tf.Variable(tf.random_normal([self.dimhidden])),  # [128]
            "b2": tf.Variable(tf.random_normal([self.dimoutput]))  # [10]
        }

        learning_rate = 0.001
        x = tf.placeholder("float", [None, self.nsteps, self.diminput])  # [count, 40, 160]
        y = tf.placeholder("float", [None, self.dimoutput])  # [count, 40]

        #                       28       10        128
        rmodel = self.RNN(x, W, b, self.nsteps, self.diminput, self.dimhidden)

        pred = rmodel["OUT"]  # [16, 40]
        ...
        with tf.Session() as sess:
            ...
            for i in range(self.total_train_times):
                batch_xs, batch_ys = self.get_batch(i, size=self.batch_size, mode="train")
                ...
                if i % self.display_step == 0:
                    ...
                    print("Train: {}/{} cost: {} accr: {}".format(i, self.total_train_times, cost_o, accr_o))
